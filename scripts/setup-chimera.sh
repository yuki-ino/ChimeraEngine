...
main() {
    ...
    # CHIMERA_PLAN.md初期化
    log_info "統一計画管理システム初期化中..."
    "${SCRIPT_DIR}/lib/plan-manager.sh" init
    
    # 並列最適化システム初期化
    log_info "並列タスク最適化システム初期化中..."
    "${SCRIPT_DIR}/lib/parallel-optimizer.sh" init
    
    # エージェントヘルスモニター初期化
    log_info "エージェント健康監視システム初期化中..."
    "${SCRIPT_DIR}/lib/agent-health-monitor.sh" init
    
    # エージェント身元確認システム初期化
    log_info "エージェント身元確認システム初期化中..."
    "${SCRIPT_DIR}/lib/agent-identity.sh" init
    
    # Claude Code認証自動化
    auto_authenticate_claude
    
    # ヘルスモニターデーモン開始
    log_info "エージェント健康監視デーモン開始中..."
    "${SCRIPT_DIR}/lib/agent-health-monitor.sh" start-daemon
    
    # 全エージェントに役割確認メッセージ送信
    log_info "🎭 全エージェントに役割確認メッセージ送信..."
    sleep 5  # Claude Code起動を待つ
    "${SCRIPT_DIR}/lib/agent-identity.sh" send-all "startup"
